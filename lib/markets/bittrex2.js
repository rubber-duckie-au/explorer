...
function get_summary(coin, exchange2, cb) {
  var req_url = base_url + '/getmarketsummary?market=' + exchange2 + '-' + coin;
...
function get_trades(coin, exchange2, cb) {
  var req_url = base_url + '/getmarkethistory?market=' + exchange2 + '-' + coin + '&count=50';
  request({uri: req_url, json: true}, function (error, response, body) {
...
function get_orders(coin, exchange2, cb) {
  var req_url = base_url + '/getorderbook?market='  + exchange2 + '-' + coin + '&type=both' + '&depth=50';
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.success == true) {
...
module.exports = {
  get_data: function(settings, cb) {
    var error = null;
    get_orders(settings.coin, settings.exchange2, function(err, buys, sells) {
      if (err) { error = err; }
      get_trades(settings.coin, settings.exchange2, function(err, trades) {
        if (err) { error = err; }
        get_summary(settings.coin, settings.exchange2, function(err, stats) {
          if (err) { error = err; }
